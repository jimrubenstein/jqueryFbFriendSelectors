...
//log and google event tracker
(function($){var c=window.console?true:false;$.log=function(m){if(c){console.log(m);return m;}return false;};$.goog={trackEvent:function(category,action,label,integer){if(window.pageTracker){try{if(category&&action&&label&&integer){pageTracker._trackEvent(category,action,label,integer);}else if(category&&action&&label){pageTracker._trackEvent(category,action,label);}else if(category&&action){pageTracker._trackEvent(category,action);}else{return false;}return true;}catch(e){}}}}})(jQuery);
//boxy
